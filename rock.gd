...
func _drop_ore() -> void:
	...
	tween.tween_property(ore, "position:y", ore.position.y - 20, 0.3).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_QUAD)
	tween.tween_property(ore, "position:y", ore.position.y, 0.3).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_QUAD).set_delay(0.3)
	
	tween.tween_property(ore, "position:x", target_x, 0.6).set_ease(Tween.EASE_OUT)
